...
function aoscpremote()
{
    ...
    git remote rm aoscp 2> /dev/null
    ...
    local AOSCP_USER=$(git config --get gerrit.cypheros.co.username)
    if [ -z "$AOSCP_USER" ]
    then
        git remote add aoscp ssh://gerrit.cypheros.co:29418/$PFX$GERRIT_REMOTE
    else
        git remote add aoscp ssh://$AOSCP_USER@gerrit.cypheros.co:29418/$PFX$GERRIT_REMOTE
    fi
    echo "Remote 'aoscp' created"
}
...
function makerecipe() {
    ...
    then
        pwd
        aoscpremote
        git push aoscp HEAD:refs/heads/'$1'
    fi
    '
}
...
function aoscpgerrit() {
    ...
    local user=`git config --get gerrit.cypheros.co.username`
...
function aoscprebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CypherOS Gerrit Rebase Usage: "
